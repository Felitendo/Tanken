...
/// Loads `/api/history` for the selected country and filters it client-side by range,
/// mirroring the web's Zeitraum chips (7/14/30 days, 0 = everything).
@MainActor
@Observable
final class HistoryViewModel {
    var country: Country = .de
    /// Selected range in days; 0 means "Alles".
    var rangeDays = 7
    ...
    func load(api: ApiClient) async {
        ...
        failed = false
        do {
            let response = try await api.history(country: country)
